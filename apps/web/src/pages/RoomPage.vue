  ...
  <div class="min-h-screen bg-primary text-text-primary">
    ...
    >
      <div class="max-w-[1440px] mx-auto h-full px-4 sm:px-8 flex items-center justify-between">
        <div class="flex items-center gap-3">
          ...
          <span class="text-sm font-semibold">{{ $t('room.roomLobby') }}</span>
          <span
        ...
        <div class="flex items-center gap-2">
          <button
            ...
            @click="shareInvite"
          >
            <Share2 class="w-3.5 h-3.5" />{{ $t('room.invite') }}
          </button>
          <button
            ...
            @click="leaveRoom"
          >
            <LogOut class="w-3.5 h-3.5" />{{ $t('room.leave') }}
          </button>
        </div>
      </div>
    </nav>
    ...
    <main class="pt-20 max-w-[1440px] mx-auto px-4 sm:px-8 pb-12">
      ...
      <template v-else>
        <!-- 房间头 -->
        <div
          class="backdrop-blur-xl bg-glass border border-glass-border rounded-2xl p-5 mb-6 flex items-center justify-between"
        >
          <div class="flex items-center gap-4">
            <div
              class="w-12 h-12 rounded-xl bg-gradient-to-br from-gold/20 to-gold/5 border border-gold/30 flex items-center justify-center"
            >
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <section class="lg:col-span-8 space-y-6">
            <!-- VS Arena -->
            <div
              class="backdrop-blur-xl bg-glass border border-glass-border rounded-2xl p-8 relative overflow-hidden"
            >
              <div
              ...
              <div class="relative grid grid-cols-[1fr_auto_1fr] gap-6 items-center">
                <!-- 黑座（房主） -->
                <div class="flex flex-col items-center text-center">
                  ...
                  >
                    <div
                      class="w-24 h-24 rounded-full bg-gradient-to-br from-gray-700 to-gray-900 border-4 flex items-center justify-center shadow-[0_8px_32px_rgba(0,0,0,0.5)]"
                      :class="blackReady ? 'border-emerald-400/60' : 'border-glass-border'"
                    >
                      <span class="text-3xl font-bold text-white">{{ $t('room.blackMark') }}</span>
                    </div>
                    <div
                      v-if="blackReady"
                  ...
                  <span
                    class="mt-3 inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-medium border"
                    :class="
                      blackReady
                ...
                <div class="flex flex-col items-center px-4">
                  <div
                    class="w-14 h-14 rounded-full bg-gradient-to-br from-board-green to-[#0d4a28] flex items-center justify-center shadow-[0_4px_20px_rgba(26,107,60,0.4)] animate-[float_4s_ease-in-out_infinite]"
                  >
                    <span class="text-lg font-black text-white">VS</span>
                  </div>
                ...
                <div class="flex flex-col items-center text-center">
                  ...
                  >
                    <div
                      class="w-24 h-24 rounded-full bg-gradient-to-br from-white to-gray-200 border-4 flex items-center justify-center shadow-[0_8px_32px_rgba(0,0,0,0.3)]"
              ...
              <!-- 准备/开局栏 -->
              <div
                class="relative mt-8 pt-6 border-t border-glass-border flex items-center justify-between flex-wrap gap-3"
              >
                <div class="flex items-center gap-2 text-xs text-text-secondary">
                  <Info class="w-3.5 h-3.5 text-gold" />
                  <span>
            ...
            <div class="backdrop-blur-xl bg-glass border border-glass-border rounded-2xl p-6">
              ...
              <div class="grid grid-cols-2 gap-x-8 gap-y-5">
                <div class="flex items-center justify-between">
                  ...
                  <div
                    class="flex items-center gap-1 bg-[rgba(255,255,255,0.03)] border border-glass-border rounded-full p-1"
                  >
                    <span
                ...
                <div class="flex items-center justify-between">
                  ...
                  <span class="text-xs text-gold font-medium px-2.5 py-1 bg-gold/10 rounded-full">{{
                    $t('room.threeMin')
                  }}</span>
                </div>
                <div class="flex items-center justify-between">
                  ...
                  <button
                    ...
                    class="px-3 py-1 rounded-full text-[11px] font-medium bg-gold/10 text-gold border border-gold/20 hover:bg-gold/20 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                    @click="swapColors"
                  >
                    {{ $t('room.swapColors') }}
                  </button>
                  <span
                    v-else
                    class="text-xs text-gold font-medium px-2.5 py-1 bg-gold/10 rounded-full"
                    >{{ $t('room.fixed') }}</span
                  >
                </div>
                <div class="flex items-center justify-between">
                  ...
                  <button
                    ...
                    class="w-11 h-6 rounded-full p-0.5 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                  ...
                  <div
                    v-else
                    class="w-11 h-6 rounded-full p-0.5"
                    :class="spectatable ? 'bg-board-green' : 'bg-glass-border'"
                  >
                    <div
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <template v-if="isHost">
                      <input
                        ...
                        class="w-32 bg-[rgba(255,255,255,0.03)] border border-glass-border rounded-lg py-1 px-2 text-xs text-text-primary focus:outline-none focus:border-gold/30"
            ...
            <div class="backdrop-blur-xl bg-glass border border-glass-border rounded-2xl p-5">
              ...
              <div v-else class="flex items-center gap-3 flex-wrap">
                <div
                  ...
                  :key="s.userId"
                  class="flex items-center gap-2 px-3 py-2 rounded-xl bg-[rgba(255,255,255,0.03)] border border-glass-border"
                >
                  <div
